"""
KOSPI200 종목별 2026년 3월 뉴스 기사 20건씩 수집 (1회 실행용)
"""
import csv
import os
import re
...
# ── 본문 전처리 (naver_stock_news_scraper.py 동일) ────────────
_KNOWN_MEDIA_SN = (
...
# ── 종목별 3월 기사 크롤링 ────────────────────────────────────
def crawl_march_news(stock, target=TARGET_PER_STOCK):
    """
    finance.naver.com PC 웹에서 3월 기사를 최대 target건 수집.
    페이지당 약 10~13건 반환. 최신순이므로 3월 이전 기사가 나오면 중단.
    """
    ...
    while len(valid) < target:
        url = NEWS_LIST_URL.format(code=code, page=page)
        try:
            r = requests.get(url, headers=PC_HEADERS, timeout=10)
            r.raise_for_status()
            r.encoding = 'euc-kr'
        except Exception as e:
            print(f"    [{code}] 목록 요청 실패 (page {page}): {e}")
            break

        soup = BeautifulSoup(r.text, 'html.parser')
        # class="relation_tit" 인 tr만 선택 → 대표 기사 행만, 연관 기사 묶음 제외
        rows = soup.select('tr.relation_tit')
        if not rows:
            break
...
if __name__ == "__main__":
    ...
    all_stocks = load_kospi200(os.path.join(base_dir, "kospi200_targets.csv"))

    # 263750 종목만 필터링
    stocks = [s for s in all_stocks if s["stock_code"] == "352820"]
    if not stocks:
        print("263750 종목을 CSV에서 찾을 수 없습니다.")
        exit(1)

    print(f"종목 263750({stocks[0]['stock_name']}) — 3월 기사 최대 {TARGET_PER_STOCK}건 수집 시작")
    total_saved = 0

    for i, stock in enumerate(stocks, 1):
        ticker = stock["ticker"]
        articles = crawl_march_news(stock)

        if not articles:
            print(f"  [{i}/{len(stocks)}] [{ticker}] {stock['stock_name']} — 3월 기사 없음")
            time.sleep(0.5)
            continue

        # DB 중복 제거 (이미 저장된 기사는 요약 전에 제외)
        articles = deduplicate(articles)
        if not articles:
            print(f"  [{i}/{len(stocks)}] [{ticker}] {stock['stock_name']} — 모두 중복")
            time.sleep(0.5)
            continue
        ...
        # 저장 (중복은 MongoDB unique index가 조용히 스킵)
        try:
            result = collection.insert_many(articles, ordered=False)
            saved = len(result.inserted_ids)
        except Exception as e:
            # BulkWriteError: 일부 중복 → inserted_ids로 실제 저장 수 확인
            saved = getattr(e, "details", {}).get("nInserted", 0)

        total_saved += saved
        print(f"  [{i}/{len(stocks)}] [{ticker}] {stock['stock_name']} — {saved}건 저장 ({len(articles)}건 수집)")
        time.sleep(0.5)
