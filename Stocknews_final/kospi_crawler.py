"""
KOSPI200 네이버 국내 주식 뉴스 크롤러
- CSV: kospi200_targets.csv (stock_code, stock_name)
- URL: https://m.stock.naver.com/domestic/stock/{stock_code}/news
...
# ── 뉴스 목록 수집 ────────────────────────────────────────────
def fetch_news_list(code, target=20):
    items = []
    page = 1
    page_size = 20

    while len(items) < target:
        url = NEWS_API_URL.format(code=code, page_size=page_size, page=page)
        try:
            r = requests.get(url, headers=MOBILE_HEADERS, timeout=10)
            r.raise_for_status()
            data = r.json()
        except Exception as e:
            print(f'  [{code}] API 요청 실패 (page {page}): {e}')
            break
        ...
        if not batch:
            print(f'  [{code}] page {page}: 기사 없음 → 종료')
            break
...
def fetch_news_list_from_page(code, target=20):
    """__NEXT_DATA__ 파싱 폴백"""
    url = LOCAL_NEWS_URL.format(code=code)
    try:
...
def crawl_stock_news(stock, target=20):
    ...
    raw_items = fetch_news_list(stock_code, target=target)

    if not raw_items:
        print(f'  [{stock_code}] API 결과 없음 → 페이지 파싱 시도')
        raw_items = fetch_news_list_from_page(stock_code, target=target)
...
def load_kospi200(filepath):
    """kospi200_targets.csv 로드 (stock_code, stock_name)"""
    stocks = []
...
if __name__ == '__main__':
    ...
    csv_path = os.path.join(base_dir, 'kospi200_targets.csv')

    stocks = load_kospi200(csv_path)
    TARGET = 20

    print(f'[KOSPI200 뉴스] {len(stocks)}종목 — 최신 {TARGET}건씩 수집 시작')
    total_saved = 0

    for i, stock in enumerate(stocks, 1):
        code = stock['stock_code']
        print(f'\n[{i}/{len(stocks)}] {code} ({stock["stock_name"]}) 수집 중...')
